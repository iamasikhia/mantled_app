...
class MAFinancialAssetDetailsScreen extends StatefulWidget {
  static String tag = '/MAFinancialAssetDetailsScreen';

  final AssetDetailModel assetData;
  final Color assetColor;
  final String assetCat;
  final String assetType;
  final String assetID;
  const MAFinancialAssetDetailsScreen({Key? key, required this.assetData, required this.assetColor, required this.assetType, required this.assetID, required this.assetCat}) : super(key: key);

  @override
  MAFinancialAssetDetailsScreenState createState() => MAFinancialAssetDetailsScreenState();
}

class MAFinancialAssetDetailsScreenState extends State<MAFinancialAssetDetailsScreen> {
  late Future assetFinancialDocuments;
  late Future collaborators;

  @override
  void initState() {
    super.initState();
     Provider.of<Data>(context, listen: false).fetchFinancialDocumentList(context, widget.assetID, widget.assetCat);
    assetFinancialDocuments = getAssetDocumentList();

    Provider.of<Data>(context, listen: false).fetchAFinancialAssetCollabList(context, widget.assetID, widget.assetCat);
    collaborators = getAssetCollabList();

  }

  Future getAssetDocumentList() async {
    await Provider.of<Data>(context, listen: false).fetchFinancialDocumentList(context, widget.assetID,  widget.assetCat);
    var c = Provider.of<Data>(context, listen: false).assetFinancialDocuments;
    return c;
  }

  Future getAssetCollabList() async {
    await Provider.of<Data>(context, listen: false).fetchAFinancialAssetCollabList(context, widget.assetID, widget.assetCat);
    var c = Provider.of<Data>(context, listen: false).financialCollaborator;
    return c;
  }
  ...
  void _show(BuildContext ctx, AssetDocumentModel assetDoc) {
        ...
        builder: (ctx) => Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () {
                  print(assetDoc.assetUrl!);
                  if(assetDoc.assetUrl!.contains("pdf")){
                    Navigator.push(
                        context,
                        MaterialPageRoute<dynamic>(
                          builder: (_) =>   PDFViewerFromUrl(
                            url: assetDoc.assetUrl!,
                            pdfName: assetDoc.assetName!,
                          ),
                        ));

                  }
                  else if(assetDoc.assetUrl!.contains("jpg")||
                      assetDoc.assetUrl!.contains("png")){
                    openImagesPage(Navigator.of(context),
                      imgUrls: [assetDoc.assetUrl!,assetDoc.assetUrl!],
                    );
                  }
              ...
              GestureDetector(
                onTap: () {
                  initiateDeleteDocument(assetDoc.assetID);
                },
                child: Column(
                  children: [
                    Padding(
  ...
  Widget listView() {
    return FutureBuilder(
        future: assetFinancialDocuments,
        builder: (context, AsyncSnapshot snapshot) {
          if (snapshot.data == null) {
            return const Center(
              child: CupertinoActivityIndicator(),
